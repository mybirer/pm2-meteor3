...
(function () {
	...
	getAppSrc = module.exports = {
		...
		makeClean: function (done) {
			return exec(
				`cd ${CWD} && rm -rf ${_settings.bundleName} && rm ${_settings.pm2EnvConfigName} && rm ${_settings.bundleTarName} && rm -rf ${_settings.gitDirName}`,
				function (err, stdout, stderr) {
					if (err) {
						return done(err);
					} else {
		...
		makeCleanAndLeaveBundle: function (done) {
			return exec(
				`cd ${CWD} && rm -rf ${_settings.bundleName} && rm ${_settings.pm2EnvConfigName} && rm -rf ${_settings.gitDirName}`,
				function (err, stdout, stderr) {
					if (err) {
						return done(err);
					} else {
