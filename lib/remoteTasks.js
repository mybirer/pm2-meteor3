...
(function () {
	...
	getAppLocation = function (pm2mConf) {
		return unixPathJoin(pm2mConf.server.deploymentDir, pm2mConf.appName);
	};

	getBackupLocation = function (pm2mConf) {
		return unixPathJoin(getAppLocation(pm2mConf), _settings.backupDir);
	};
	...
	module.exports = {
		getRemoteSession: function (pm2mConf) {
			...
			if (pm2mConf.server.password) {
				...
				connectOptions.privateKey = fs.readFileSync(abs(pm2mConf.server.pem), 'utf8');
			}
		...
		prepareHost: function ({ ssh, connectOptions }, pm2mConf, done) {
			var cmd;
			cmd = cmdString(pm2mConf, `mkdir -p ${unixPathJoin(getAppLocation(pm2mConf), _settings.backupDir)}`);
			return ssh
				.connect(connectOptions)
				.then(function () {
					return ssh.execCommand(cmd);
		...
		shipTarBall: function ({ ssh, connectOptions }, pm2mConf, done) {
			...
			destination = unixPathJoin(getAppLocation(pm2mConf), _settings.bundleTarName);
		...
		shipSettings: function ({ ssh, connectOptions }, pm2mConf, done) {
			...
			destination = unixPathJoin(getAppLocation(pm2mConf), _settings.pm2EnvConfigName);
		...
		installBundleDeps: function ({ ssh, connectOptions }, pm2mConf, done) {
			...
			var cmd, installLocation, npmInstallCmd;
			var interpreter = pm2mConf.server.interpreter;
			var pathPrefix = interpreter && interpreter.trim() !== ''
				? 'export PATH="' + path.dirname(interpreter).replace(/\\/g, '/') + ':$PATH" && '
				: '';

			if (appType === 'nextjs' || appType === 'express') {
				// Next.js and Express: install in bundle directory
				installLocation = unixPathJoin(getAppLocation(pm2mConf), _settings.bundleName);
				// Detect if the bundled app requires yarn (reads the just-created bundle locally,
				// before it ships — we read from CWD/bundleName/package.json).
				var localBundlePkg = path.join(CWD, _settings.bundleName, 'package.json');
				var useYarn = false;
				try {
					if (fs.existsSync(localBundlePkg)) {
						var pkg = JSON.parse(fs.readFileSync(localBundlePkg, 'utf8'));
						var eng = pkg && pkg.engines;
						if (eng && ((typeof eng.npm === 'string' && /please-use-yarn/i.test(eng.npm)) || eng.yarn)) {
							useYarn = true;
						}
						if (!useYarn && pkg && typeof pkg.packageManager === 'string' && pkg.packageManager.startsWith('yarn')) {
							useYarn = true;
						}
					}
					if (!useYarn && fs.existsSync(path.join(CWD, _settings.bundleName, 'yarn.lock'))) {
						useYarn = true;
					}
				} catch (e) {
					useYarn = false;
				}
				console.log(`Installing dependencies for ${appType} on server (${useYarn ? 'yarn' : 'npm'})...`);
				if (useYarn) {
					npmInstallCmd = pathPrefix + `cd ${installLocation} && node --version && yarn install --production`;
				} else {
					npmInstallCmd = pathPrefix + `cd ${installLocation} && node --version && npm install --production --legacy-peer-deps`;
				}
				cmd = cmdString(pm2mConf, npmInstallCmd);
			} else if (appType === 'vite') {
				// Vite: install serve only (minimal package.json)
				installLocation = unixPathJoin(getAppLocation(pm2mConf), _settings.bundleName);
				console.log('Installing serve for Vite app on server...');
				npmInstallCmd = pathPrefix + `cd ${installLocation} && node --version && npm install --production`;
				cmd = cmdString(pm2mConf, npmInstallCmd);
			} else {
				// Meteor: install in programs/server directory
				installLocation = unixPathJoin(getAppLocation(pm2mConf), _settings.bundleName, 'programs/server');
				npmInstallCmd = pathPrefix + `cd ${installLocation} && node --version && npm install --production`;
				cmd = cmdString(pm2mConf, npmInstallCmd);
			}
			...
			return ssh
				...
				.then(function (result) {
					...
					if (result.stderr) {
						...
						if (result.stderr.includes('ERR!') || result.stderr.includes('error:')) {
			...
			return ssh
				...
				.then(function (result) {
					if (result.stderr) {
						...
						if (result.stderr.includes('[PM2][ERROR]') || result.stderr.includes('Error:')) {
							return done(new Error(result.stderr));
						}
					}
					return done();
		...
		hardReloadApp: function ({ ssh, connectOptions }, pm2mConf, done) {
			...
			return ssh
				...
				.then(function (result) {
					...
					if (result.stderr) {
						console.log(result.stderr);
						// pm2 delete may fail if app doesn't exist, that's ok for first deploy
					}
		...
		revertToBackup: function ({ ssh, connectOptions }, pm2mConf, done) {
			...
			cmd = cmdString(
				pm2mConf,
				`mv ${unixPathJoin(backupLocation, _settings.bundleTarName)} ${unixPathJoin(appLocation, _settings.bundleTarName)}`,
			);
